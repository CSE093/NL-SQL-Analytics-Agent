...
def run_query(sql_query: str) -> Dict[str, Any]:
    """
    Validates and runs a SQL query on the database.
    ...
    """
    logger.info(f"Validating and executing SQL query: {sql_query}")
    
    # 1. Validate the SQL query first
    ...
    try:
        df, exec_time = execute_query(sql_query)
        logger.info(f"SQL query executed successfully in {exec_time:.2f}ms. Rows returned: {len(df)}")
        return {
            "success": True,
            "data": df,
            "error": None,
